...
function status(id, es, en = es) {
  send("status", { id, es, en });
}
...
function wavBuffer(samples, sampleRate = SAMPLE_RATE) {
...
async function loadRuntime(id) {
  ...
  runtimePromise = (async () => {
    status(id, "Cargando motor de voz natural…", "Loading natural voice engine…");
    ...
    const progress_callback = (p) => {
      ...
      status(id, `Descargando modelo natural… ${pct}%`, `Downloading natural model… ${pct}%`);
    };
...
async function getVoiceData(id, voice) {
  if (voiceMemory.has(voice)) return voiceMemory.get(voice);
  const url = `${VOICE_BASE}/${voice}.bin`;
  status(id, "Cargando perfil de voz…", "Loading voice profile…");
...
async function synthPart(id, text, voice, runtime) {
  ...
  if (!phonemes.trim()) throw new Error("No se pudo fonetizar el texto / Could not phonemize the text");
  ...
  const allVoiceData = await getVoiceData(id, voice);
  const offset = numTokens * STYLE_DIM;
  const styleData = allVoiceData.slice(offset, offset + STYLE_DIM);
  if (styleData.length !== STYLE_DIM) throw new Error("El perfil de voz no es válido / Invalid voice profile");

  status(id, "Generando audio…", "Generating audio…");
  const inputs = {
    input_ids,
...
async function synth(id, text, cfg) {
  ...
  if (!parts.length) return wavBuffer(new Float32Array(1));

  const audioParts = [];
  let total = 0;
  for (let i = 0; i < parts.length; i++) {
    status(id, `Generando voz natural… ${i + 1}/${parts.length}`, `Generating natural voice… ${i + 1}/${parts.length}`);
    const samples = await synthPart(id, parts[i], voice, runtime);
